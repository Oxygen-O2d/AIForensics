...
def generate_pdf_buffer(results):
    ...
    if os.path.exists("temp_thumb.jpg"):
        ...
        c.setLineWidth(0.5)
        c.rect(40, y-180, 180, 180, fill=0, stroke=1) 
    ...
    c.setFont("Helvetica", 10)
    c.setFillColor(text_secondary)
    c.drawString(40, y, "Individual frame manipulation scores:")
    ...
    if num_frames > 0:
        ...
        for i, score in enumerate(frame_scores):
            bx = 40 + i * (box_width + 5)
            by = y
            if score > 0.6: box_color = colors.HexColor("#ef4444")
            elif score > 0.35: box_color = colors.HexColor("#eab308")
            else: box_color = colors.HexColor("#22c55e")
    ...
    c.drawRightString(w-40, 20, f"Generated by AI Forensic Console // Page 1 of 1")
